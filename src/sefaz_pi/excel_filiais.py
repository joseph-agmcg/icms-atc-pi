"""
Extração de dados de planilhas Excel de filiais (formato ICMS Antecipado / ICMS ATC).
Planilhas no formato APURAÇÃO DE ICMS PIAUI.
Captura todos os dados: todas as colunas do cabeçalho e todas as linhas até a linha de TOTAL
ou até a primeira linha com I.E. vazia / rodapé (ex.: "PIAUI", "Apresentação ao Fisco").
O período de referência é lido da área do título (data ou mês/ano), não de uma coluna.
Valor principal: coluna ATC (ICMS Antecipado); I.E. na coluna INSC.ESTADUAL (ou sinônimos).
"""
...
def _extrair_periodo_da_area_titulo(planilha: openpyxl.worksheet.worksheet.Worksheet) -> tuple[int, int]:
    ...
    raise ValueError(
        ...
        "Inclua a data (ex.: 1/1/2026) ou mês/ano (ex.: jan-26) na área do título (ex.: APURAÇÃO DE ICMS PIAUI)."
    )
...
def _valor_ie_para_string(valor: str | int | float | None) -> str | None:
    """Converte o valor da célula I.E. para string (pode vir número ou texto, ex.: 19.472.058-6)."""
    if valor is None:
        return None
    if isinstance(valor, (int, float)):
    ...
    if not s:
        return None
    apenas_digitos = "".join(c for c in s if c.isdigit())
    if not apenas_digitos:
        return None
    return _normalizar_ie_zeros_esquerda(apenas_digitos)


def _normalizar_ie_zeros_esquerda(ie: str) -> str:
    """Se a I.E. for só dígitos e tiver menos que DIGITOS_IE_PI caracteres, preenche com zeros à esquerda."""
    if not ie or len(ie) >= DIGITOS_IE_PI:
        return ie
    if not ie.isdigit():
        return ie
    return ie.zfill(DIGITOS_IE_PI)


def _eh_linha_fim_dados(valor_ie: object) -> bool:
    """Indica se o valor na coluna I.E. é total/footer ou vazio (parar de ler)."""
    if valor_ie is None:
        return True
    v = str(valor_ie).strip().upper()
    if not v:
        return True
    return v.startswith("TOTAL") or v == "PIAUI"
...
def _obter_chave_normal(nome_para_indice: dict[str, int]) -> str | None:
    """Retorna a chave do cabeçalho que corresponde à coluna NORMAL (outros formatos; para ICMS Antecipado usar ATC)."""
...
def _obter_chave_atc(nome_para_indice: dict[str, int]) -> str | None:
    """Retorna a chave do cabeçalho que corresponde à coluna ATC (valor principal para ICMS Antecipado)."""
    n = _normalizar_cabecalho("ATC")
    for chave in nome_para_indice:
        if _normalizar_cabecalho(chave) == n:
            return chave
    return None
...
def ie_apenas_digitos(ie: str) -> str:
    ...
    return digitos.zfill(DIGITOS_IE_PI) if len(digitos) <= DIGITOS_IE_PI else digitos[:DIGITOS_IE_PI]
...
def obter_dados_para_dae(
    ...
    """
    Retorna uma lista de dict por linha, com: ie, ie_digitos, valor_atc, mes_ref, ano_ref.
    O valor principal (valor_atc) é lido da coluna ATC da planilha (ICMS Antecipado).
    """
    ...
    ie_key = _obter_chave_ie(nome_para_indice)
    atc_key = _obter_chave_atc(nome_para_indice)
    if ie_key is None:
        raise ValueError("Coluna I.E. não encontrada no mapeamento. Nenhum dado para processar.")
    if atc_key is None:
        raise ValueError("Coluna ATC não encontrada no mapeamento. Adicione a coluna ATC na planilha.")
    ...
    for row in linhas:
        ...
        if not ie_str:
            continue
        valor_atc: float | None = None
        v = row.get(atc_key)
        if isinstance(v, (int, float)):
            valor_atc = float(v)
        elif v is not None and str(v).strip():
            try:
                valor_atc = float(str(v).strip().replace(",", "."))
            except ValueError:
                valor_atc = None
        resultado.append({
            "ie": ie_str,
    ...
    logger.info("Dados para ICMS Antecipado: %d registro(s) (período %02d/%d).", len(resultado), mes_ref, ano_ref)
    return resultado
...
def obter_ies_dos_dados(
    ...
    """Extrai a lista de I.E. a partir dos dados já carregados."""
    ie_key = _obter_chave_ie(nome_para_indice)
    if ie_key is None:
        return []
    ies: list[str] = []
