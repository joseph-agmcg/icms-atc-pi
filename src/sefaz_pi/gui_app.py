"""
Interface desktop com CustomTkinter para ICMS Antecipado (SEFAZ-PI).
Upload de arquivo Excel → extração de I.E. e coluna ATC → detalhamento (executáveis x ignoradas)
...
from sefaz_pi.automacao_sefaz_pi import AutomacaoAntecipacaoParcialPI, _valor_atc_invalido
from sefaz_pi.excel_filiais import extrair_todos_os_dados, obter_dados_para_dae, obter_ies_dos_dados
from sefaz_pi.excel_filiais import _obter_chave_ie
...
def _contar_executaveis_ignoradas(lista_dados: list[dict[str, object]]) -> tuple[int, int]:
    ...
    executaveis = sum(1 for item in lista_dados if not _valor_atc_invalido(item.get("valor_atc")))
...
def _executar_lote_em_background(
    ...
    async def _rodar() -> tuple[list[str], list[tuple[str, str]]]:
        automacao = AutomacaoAntecipacaoParcialPI(headless=headless)
        return await automacao.executar_fluxo_por_ie_pi(lista_dados)
...
def _mostrar_janela_resultado(
    ...
    janela.title("Resultado da automação — ICMS Antecipado (SEFAZ-PI)")
...
class JanelaSelecaoIEs(ctk.CTkToplevel):
    """
    Janela de detalhamento: lista todas as IEs com valor ATC e status (executável / será ignorada).
    ...
    def __init__(
        ...
        self.title("Escolher IEs para executar — ICMS Antecipado (SEFAZ-PI)")
        ...
        ctk.CTkLabel(
            self,
            text=f"Total: {total} I.E.(s)  |  Com valor ATC (executáveis): {qtd_exec}  |  Sem valor (ignoradas): {qtd_ign}",
            font=ctk.CTkFont(size=12),
            text_color="gray",
        ).pack(pady=(12, 4))
        ...
        ctk.CTkLabel(
            self,
            text="Marque as I.E. que deseja executar. I.E. sem valor ATC não podem ser marcadas.",
            font=ctk.CTkFont(size=11),
            text_color="gray",
        ).pack(pady=(0, 8))

        # Frame rolável com linhas: [checkbox] IE | Valor ATC | Status
...
class AppFiliais(ctk.CTk):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.title("ICMS Antecipado — SEFAZ-PI")
        ...
        self._lista_dados: list[dict[str, object]] = []  # ie, ie_digitos, valor_atc, mes_ref, ano_ref
    ...
    def _criar_widgets(self) -> None:
        ...
        # Título e opção do fluxo
        self._titulo = ctk.CTkLabel(
            self,
            text="ICMS Antecipado (SEFAZ-PI)",
            font=ctk.CTkFont(size=16, weight="bold"),
        )
        self._titulo.pack(**padding)
        ctk.CTkLabel(
            self,
            text="Upload do arquivo Excel (filiais) — coluna INSC.ESTADUAL e ATC",
            font=ctk.CTkFont(size=12),
            text_color="gray",
        ).pack(pady=(0, 8))
    ...
    def _ao_selecionar_arquivo(self) -> None:
        ...
        texto = (
            ...
            f"Com valor ATC (executáveis): {qtd_exec}. Sem valor (ignoradas): {qtd_ign}."
        )
    ...
    def _ao_executar_todas(self) -> None:
        """Executa automação para todas as IEs com valor ATC (executáveis)."""
        if self._em_execucao or not self._lista_dados:
            return
        # Apenas IEs executáveis (com valor ATC)
        lista_executaveis = [
        ...
        if not lista_executaveis:
            messagebox.showwarning(
                "Aviso",
                "Nenhuma I.E. com valor ATC para executar. Use 'Escolher IEs e executar' para ver o detalhamento.",
            )
            return
...
def main() -> None:
    ...
    logger.info("Iniciando interface ICMS Antecipado (SEFAZ-PI).")
    app = AppFiliais()
    app.mainloop()
